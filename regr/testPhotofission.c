...
int main ()
{
   ...
   if (fp == 0) {
      fp = fopen("testPhotofissionNuDist.res", "w");
      if (fp == (FILE *) 0) fprintf(stderr, "Could not open testPhotofissionNuDist.res for writing");
   }
   if (fpspec == 0) {
      fpspec = fopen("testPhotofissionSpec.res", "w");
      if (fpspec == (FILE *) 0) fprintf(stderr, "Could not open testPhotofissionSpec.res for writing");
   }
   for (i=0; i<4; i++) {
      setnudist_(&number[i]);
