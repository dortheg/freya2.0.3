...
plot \
"../regr/testspec.goldref" using 4:((column(1) == iso && column(3) == engWatt && strcol(2) eq 'n')?column(5):1/0) with linespoints title sprintf("Original Watt spectrum, isotope = %g engWatt = %g MeV", iso, engWatt), \
"../regr/testspec.goldref" using 4:((column(1) == iso && column(3) == engWatt && strcol(2) eq 'p')?column(5):1/0) with linespoints title sprintf("Original fission gamma spectrum, isotope = %g engWatt = %g MeV", iso, engWatt), \
