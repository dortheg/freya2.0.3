...
int main() {
   ...
   //output_compound(fp, Z, A+((fissiontype==0)?0:1), (fissiontype==0)?0.:energy_MeV, iterations); //new, unsure if works
...
void readinput(int& Z, int& A, double& E, int& fissiontype, int& iterations, char outputfilename [1024]) {
  ...
  if (-1==fissiontype){
    cout << iterations << " specified excitation of " << Z << A << endl;
  }
  if (fissiontype==1) {
