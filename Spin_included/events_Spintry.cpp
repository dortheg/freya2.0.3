...
//9.feb with angular momentum input from user for photofission included

extern "C" {
   extern int msfreya_setup_c_();
   extern int msfreya_event_c_(int,double,double,double,double*,int*,int*,double*,int*,int*,double*,int*,double*,int*,double*);
...
bool FREYA_event(FILE* fp, int Z, int A, int fissionindex, double ePart, double S,
...
void output_secondaries(FILE* fp, int ptypes [mMax], double particles [4*3*mMax], 
                        int npart2skip);
void readinput(int& Z, int& A, double& E, int& fissiontype, double& S, int& iterations, char outputfilename [1024]);
...
int main() {
   ...
   double S = 0;		// spin of compound nucleus, if photofission
   ...
   readinput(Z, A, energy_MeV, fissiontype, S, iterations, outputfilename); 
   ...
   for (int i=0; i<iterations; i++) {
      if (!FREYA_event(fp, Z, A, i, energy_MeV, S,fissiontype, *ZAs, *fistypes, niso)) {
...
bool FREYA_event(FILE* fp, int Z, int A, int fissionindex, double ePart, double S,
	...
	//Actually calculates the fission events and writes to file
   ...
   //Here, FREYA is called!
   msfreya_event_c_(iK,En,eps0,S,&(P0[0]),&Z1,&A1,&(P1[0]),&Z2,&A2,&(P2[0]),&mult,&(particles[0]),&(ptypes[0]),&(ndir[0]));
   if (msfreya_errorflagset_c_()==1) return false;
   ...
   //....print results for fission fragment #1, light fragment
   ...
   //....print results for fission fragment #2, heavy fragment
...
double FREYA_event_eps0(FILE* fp, int Z, int A, double ePart, 
                 int fissiontype, int*& ZAs, int*& fistypes, int niso){
	//ONLY CALCULATES eps0, NOT the one that actually calculates the fission events
   int isotope = 1000*Z+A;
   // if the compound nucleus is ZA, the original nucleus was
   //   ZA for photofission
   //   Z(A-1) for neutron-induced fission
...
void readinput(int& Z, int& A, double& E, int& fissiontype, double& S, int& iterations, char outputfilename [1024]) {
  ...
  cin >> E;
  cout << "If photofission, specify value of spin (else, put 0): ";
  cin >> S;
