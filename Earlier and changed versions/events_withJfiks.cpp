...
//changed version, med jorgenfiksing, 11.jan, som ikke funket. Komplisert greie


extern "C" {
   extern int msfreya_setup_c_();
   extern int msfreya_event_c_(int,double,double,double*,int*,int*,double*,double*,int*,int*,double*,double*,int*,double*,int*,double*);
...
bool FREYA_event(FILE* fp, int Z, int A, int fissionindex, double ePart, 
   ...
   cout << "hello" << endl;
   msfreya_event_c_(iK,En,eps0,&(P0[0]),&Z1,&A1, &eps1,&(P1[0]),&Z2,&A2,&eps2,&(P2[0]),&mult,&(particles[0]),&(ptypes[0]),&(ndir[0]));
   if (msfreya_errorflagset_c_()==1) return false;
   ...
   if (msfreya_errorflagset_c_()==1) return false;
   output_ff(fp, fissionindex+1, Z1, A1, eps1, nmultff1, gmultff1, P1);
   ...
   if (msfreya_errorflagset_c_()==1) return false;
   output_ff(fp, fissionindex+1, Z2, A2, eps2, nmultff2, gmultff2, P2);
