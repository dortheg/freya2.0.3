...
{  
  ...
  SampleFission(theTrack, &nPrompt, &gPrompt, eKinetic);

  // Build two fission fragments
  G4double momentum;
  G4int Aff, Zff;
  G4int sumAff=0;
  G4double sumKEff=0;
  ...
  for (G4int i=0; i<2; i++) {
    ...
    fe->getFissionFragment(i, Aff, Zff, KEff, dir);
    sumAff+=Aff;
    if (-1==Aff) {
      continue;
    }
    it->SetDefinition( G4IonTable::GetIonTable()->GetIon( Zff, Aff, 0) );
    it->SetKineticEnergy(KEff*MeV);
    sumKEff+=KEff;
    momentum = it->GetTotalMomentum();
    G4ThreeVector temp(momentum*dir[0], 
                       momentum*dir[1], 
                       momentum*dir[2]);
    it->SetMomentum( temp );
...
void G4FissionLibrary_new::SampleFission(const G4HadProjectile & theTrack, G4int* nPrompt,
                                   G4int* gPrompt, G4double eKinetic)
{
   ...
   if (3 == fe->getCorrelationOption()) {
     ...
     if (err_len>1) {
       ...
       G4Exception("G4FissionLibrary_new::SampleFission", "freya001", FatalException,
                   ed);
     }
     delete [] error_message;
   }
